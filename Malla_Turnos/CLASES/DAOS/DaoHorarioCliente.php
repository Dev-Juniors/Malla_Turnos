...
class DaoHorarioCliente {
	...
	public static function consultarDetalleHorario($idHorarioCliente) {
		$connClass = new Conexion ();
		$conn = $connClass->getConection ();
		if ($conn != null) {
			$sql = "SELECT * FROM `detalle_horario` WHERE `id_horario_cliente` = ?";
			if ($stmt = $conn->prepare ( $sql )) {
				$stmt->bind_param ( "i", $idHorarioCliente );
				$stmt->execute ();
				$result = $stmt->get_result ();
				$campos = array (
						0 => 'id',
						1 => 'id_horario_cliente',
						2 => 'dias',
						3 => 'hora_inicio',
						4 => 'hora_fin' 
				);
	...
	public function guardarDetalleHorario($inicio, $fin, $dias) {
		...
		$conn = $connClass->getConection ();
		if ($conn != null) {
			if ($stmt = $conn->prepare ( "INSERT INTO `detalle_horario`(`id_horario_cliente`, `dias`, `hora_inicio`, `hora_fin`) VALUES (?,?,?,?)" )) {
